export default class BitBuffer {
  /**
   * BitBuffer class is the core class for computational process, shifting.
   *
   ...
   * @param _left - Left part of `_buffer`, type of this property is `BitBuffer` and the size is half of the `_buffer`.
   * @param _right - Right part of `_buffer`, type of this property is `BitBuffer` and the size is half of the
   *   `_buffer`.
   */
  ...
  public set buffer(value: number[]) {
    ...
    this.right = new BitBuffer(this._buffer.slice(this.length / 2, this.length));
  }
  ...
  public constructor(param1: number[] | number, param2?: number[]) {
    if (Array.isArray(param1)) {
      if (param2 === undefined) {
        ...
        if (param1.length !== param2.length) throw new Error('Left and right halves must be of equal length');
  ...
  private static checkLength(length: number): void {
    ...
    const maxLength = 10;
    if (length > maxLength) throw new Error('Length must be less than ' + maxLength);
  }
  ...
  public shiftLeft(child: BitBuffer, position: number): BitBuffer {
    ...
    if (position > child.length) throw new Error('Shift must be less than ' + child.length);
